...
fn login(conn: MoreInterestingConn, post: Form<UserForm>, mut cookies: Cookies, user_agent: UserAgentString) -> impl Responder<'static> {
    match conn.authenticate_user(&UserAuth {
        ...
        Some(user) => {
            ...
            let cookie = Cookie::build("U", session.uuid.to_string()).path("/").secure(true).same_site(SameSite::None).finish();
            cookies.add(cookie);
...
fn logout(mut cookies: Cookies) -> impl Responder<'static> {
    let cookie = Cookie::build("U", "").path("/").secure(true).same_site(SameSite::None).finish();
    cookies.add(cookie);
    Redirect::to(".")
}
